...
func ListResourceGroupName(subscriptionId string) []string {
	rgNames := []string{}
	jsonData := listResourceGroup(subscriptionId)
	...
	var rg []map[string]interface{}
	if err := json.Unmarshal(jsonData, &rg); err != nil {
		log.Fatal(err)
	}
	for _, v := range rg {
		rgNames = append(rgNames, v["name"].(string))
	}

	return rgNames
}

func ListVirtualMachineName(subscriptionId, resourceGroupName string) []string {
	vmNames := []string{}
	jsonData := listVirtualMachine(subscriptionId, resourceGroupName)
	var rg []map[string]interface{}
	if err := json.Unmarshal(jsonData, &rg); err != nil {
		log.Fatal(err)
	}
	for _, v := range rg {
		vmNames = append(vmNames, v["name"].(string))
	}
