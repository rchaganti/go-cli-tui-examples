...
// Deletes a session and returns the new selected session
func (s *SessionStore) DeleteSession(sessionId string) (Session, error) {
	se, err := s.GetSessionEntries()
	if err != nil {
		return Session{}, err
	}
	...
	if entry.Id == "" {
		return Session{}, fmt.Errorf("session entry not found")
	}
	...
	if entry.Id == selectedSession.Id {
		// set index 0 as selected
		newSessions[0].IsSelected = true
		selectedSession = newSessions[0]
	}
