...
func (m chatModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "enter":
			...
			cmds = append(cmds, vpCmd, spCmd, m.spinner.Tick, m.addMessageToSession(message), m.generateResponse(prompt, m.messageHistory))
			m.viewState = chatview

		case "tab":
			if m.viewState == chatview {
				m.viewState = chatinput
				m.keys.view = chatinput
			} else {
				m.viewState = chatview
				m.keys.view = chatview
			}
		default:
...
func (m chatModel) View() string {
	...
	if !m.responding {
		view = lipgloss.JoinVertical(lipgloss.Top, m.headerView(), m.viewport.View(), m.footerView(), taView, m.helpView())
	} else {
		view = lipgloss.JoinVertical(lipgloss.Top, m.headerView(), m.viewport.View(), m.footerView(), spView, m.helpView())
	}
