...
func (m chatModel) footerView() string {
	...
	footerString := fmt.Sprintf("Session ID: %s | Tokens: %d", m.sessionId, m.tokenCount)
	if m.viewState == chatinput {
...
func (m chatModel) generateResponse(prompt string, history []gai.MessageContent) tea.Cmd {
	return func() tea.Msg {
		var res *genai.GenerateContentResponse
		res, err := gai.GetResponse(prompt, history, m.ai.apiKey, m.ai.model)
		if err != nil {
